...
SRC_PATH="${SRC_PATH-/src/xcloner-backup-and-restore/}"
DEST_PATH="${DEST_PATH-/var/www/html/wp-content/plugins/xcloner-backup-and-restore/}"

sync() {
    rsync "${SRC_PATH}" "${DEST_PATH}" -r --delete || true
    chown www-data:www-data -R /var/www/html
}

watcher() {
    inotifywait -e modify,create,delete,move -r "${SRC_PATH}" -q --format '%w%f' | while read FILE; do
        sync
        watcher
    done
}
