...
class Xcloner_cli {
	...
	private function encrypt_backup( $opts ) {
		...
		$this->xcloner_container
			->get_xcloner_encryption()
			->encrypt_file( $backup_name, "", $this->get_encryption_key_from_arguments( $opts ), 0, 0, true, true );
	...
	private function decrypt_backup( $opts ) {
		...
		$this->xcloner_container
			->get_xcloner_encryption()
			->decrypt_file( $backup_name, "", $this->get_encryption_key_from_arguments( $opts ), 0, 0, true );
