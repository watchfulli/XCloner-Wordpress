		...
		try {
			...
			foreach ( $files as $file ) {
				if ( ! is_readable( $this->xcloner_settings->get_xcloner_start_path() . DS . $file['path'] ) ) {
					$this->logger->info( sprintf( __( "Excluding %s from the filesystem list, file not readable" ), $file['path'] ), array(
						"FILESYSTEM SCAN",
						"NOT READABLE"
					) );
					...
					$this->logger->info( sprintf( __( "Excluding %s from the filesystem list, matching pattern %s" ), $file['path'], $matching_pattern ), array(
						"FILESYSTEM SCAN",
						"EXCLUDE"
					) );
				}
	...
	public function store_file( $file, $storage = 'start_filesystem' ) {
		...
		if ( $this->get_diff_timestamp_start() ) {
			if ( $file['type'] != "file" && $response = $this->check_file_diff_time( $file ) ) {
				$this->logger->info( sprintf( "Directory %s archiving skipped on differential backup %s", $file['path'], $response ), array(
					"FILESYSTEM SCAN",
					"DIR DIFF"
				) );
